...
parser.add_argument(
    '--num_actions', type=int, default=3, metavar='N', help='discretize action space (default: 5)')
...
class Agent():
    ...
    print("num actions = " + str(args.num_actions) + "\n")
    max_grad_norm = 0.5
...
def main():
    ...
    for i_ep in range(100):
        ...
        for t in range(200):
            ...
            env.render()
            print(str(Transition(state, action_index, (reward + 8) / 8, state_)))
...
def train_and_save():
    ...
    for episode in episodes:
        ...
        for step in range(1,len(episode)):
            ...
            prev_state = np.array([float(prev_line[1]), float(prev_line[2]), float(prev_line[4]), float(prev_line[5]), float(prev_line[7]), float(prev_line[8]), float(prev_line[10]), float(prev_line[11]), float(prev_line[13]), float(prev_line[14]), float(prev_line[16]), float(prev_line[17])])
            curr_state = np.array([float(curr_line[1]), float(curr_line[2]), float(curr_line[4]), float(curr_line[5]), float(curr_line[7]), float(curr_line[8]), float(curr_line[10]), float(curr_line[11]), float(curr_line[13]), float(curr_line[14]), float(curr_line[16]), float(curr_line[17])])
            
            
            lin = float(prev_line[18])
            ang = float(prev_line[19])
            if ang == 0.0:
                action = 1
            elif ang < 0.0:
                action = 0
            ...
            agent.store_transition(Transition(prev_state, action, reward, curr_state))

            if agent.memory.isfull:
                q = agent.update()

    torch.save(agent.target_net.state_dict(),"/home/aakaash/models/dqn_target.pth")
    torch.save(agent.eval_net.state_dict(),"/home/aakaash/models/dqn_eval.pth")

def load_and_train():
    agent = Agent()
    agent.target_net.load_state_dict(torch.load("/home/aakaash/models/dqn_target.pth"))
    agent.target_net.eval()
    agent.eval_net.load_state_dict(torch.load("/home/aakaash/models/dqn_eval.pth"))
    agent.eval_net.eval()
...
def load_and_test():
    ...
    for episode in episodes:
        ...
        for step in range(1,len(episode)):
            ...
            action_net, action_index = agent.select_action(prev_state)
            #if step_count < 200:
            print("Our Action = " + str(action) + " Net Action = " + str(action_index))

    ##plots individual rewards of each
    plt.plot(rewards)
    plt.title("Rewards")
    plt.xlabel('Line')
    plt.ylabel('Reward')
